...
function geocode(query) {
  return new Promise((resolve) => {
    ...
    https.get(url, { headers: { 'User-Agent': 'houtkaart-first/1.0' } }, (res) => {
      let d = '';
...
async function main() {
  console.log('Validating companies 0 to 151 using Photon...\n');
  const issues = [];
  let checked = 0;
  ...
  for (let i = 0; i < 152; i++) {
    ...
    if (c.adres) searchQuery = c.adres;
    else {
    ...
    checked++;
    if (!geo) { await new Promise(r => setTimeout(r, 500)); continue; }

    const drift = distKm(c.lat, c.lng, geo.lat, geo.lng);
    if (drift > 5) {
      issues.push({ index: i, naam: c.naam, stored: [c.lat, c.lng], geocoded: [parseFloat(geo.lat.toFixed(4)), parseFloat(geo.lng.toFixed(4))], driftKm: parseFloat(drift.toFixed(1)), query: searchQuery, display: geo.display });
  ...
  console.log('\n=== SUMMARY (0-151) ===');
  console.log('Checked:', checked, '| Issues (>5km):', issues.length);
  fs.writeFileSync('scripts/validation-issues-first.json', JSON.stringify(issues, null, 2));
  if (issues.length > 0) {
    console.log('\nIssues:');
    issues.forEach(i => console.log(`  [${i.index}] ${i.naam}: ${i.driftKm}km`));
  }
}
